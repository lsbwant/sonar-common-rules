...
@Rule(
  ...
  description = "<p>A violation is created on a file as soon as the comment density coverage on this file is less than the required threshold. "
    ...
    if (ResourceUtils.isEntity(resource) && commentDensity < minimumCommentDensity && linesOfCode != 0) {
      ...
      if (issuable != null) {
        Issue issue = issuable.newIssueBuilder()
          ...
          .effortToFix(missingCommentLines)
          .message((int) missingCommentLines + " more comment lines need to be written to reach the minimum threshold of "
